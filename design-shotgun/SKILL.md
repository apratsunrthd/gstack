...
Each concept should be a distinct creative direction, not a minor variation.

**Cross-model diversification (when N >= 3):** One model asked for multiple directions
tends to converge on its own stylistic priors even when told not to — real divergence
comes from different models, not different prompts to the same model. Before drafting
your own concepts, check what's available and try to source up to 2 of the N directions
externally:

```bash
_HAS_CODEX=0
command -v codex >/dev/null 2>&1 && codex login status >/dev/null 2>&1 && _HAS_CODEX=1
_HAS_GEMINI=0
if command -v gemini >/dev/null 2>&1 && { [ -n "$GEMINI_API_KEY" ] || [ -n "$GOOGLE_API_KEY" ]; }; then
  _HAS_GEMINI=1
fi
echo "codex=$_HAS_CODEX gemini=$_HAS_GEMINI"
```

If `_HAS_CODEX=1` and/or `_HAS_GEMINI=1`, run the ones that are available **in parallel**
(separate Bash tool calls in the same message, not sequential — each can take up to 90s):

Codex direction:
```bash
_REPO_ROOT=$(git rev-parse --show-toplevel)
PYTHON_CMD=$(command -v python3 2>/dev/null || command -v python 2>/dev/null)
timeout 90 codex exec "Propose ONE distinct visual design direction for: {the design brief}. One line: a name plus a description of typography, color, and layout. No code, no reasoning — just the direction." -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="medium"' --json < /dev/null 2>/tmp/codex-concept-err.txt | "$PYTHON_CMD" -c "
import sys, json
...
Gemini direction:
```bash
timeout 90 gemini -p "Propose ONE distinct visual design direction for: {the design brief}. One line: a name plus a description of typography, color, and layout. No code, no reasoning — just the direction." --output-format stream-json --yolo > /tmp/gemini-concept.ndjson 2>/tmp/gemini-concept-err.txt
jq -r 'select(.type=="message" and .role=="assistant") | .content' /tmp/gemini-concept.ndjson 2>/dev/null
```

If either errors, times out, or returns empty output, drop it silently — generate that
direction yourself instead. Don't block on or retry a failed external call; this is a
best-effort diversity boost, not a required step.

Generate the remaining directions yourself, drawing on DESIGN.md, taste memory, and the
user's request. Present all N as a lettered list, noting the source of any external
direction so the user knows where each idea came from:

```
I'll explore 3 directions:

A) "Name" — one-line visual description of this direction
B) "Name" — one-line visual description [Codex]
C) "Name" — one-line visual description [Gemini]
```

**Anti-convergence directive (hard requirement, applies to every direction regardless of
source):** Each variant MUST use a different font family, color palette, and layout
approach. If two variants look like siblings — same typographic feel, overlapping color
temperature, comparable layout rhythm — one of them failed. Regenerate the weaker one
with a deliberately different direction.
